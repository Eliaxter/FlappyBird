...
namespace Game
{
	...
	void InitPipes()
	{
		...
		tube[0].rec.height = static_cast<float>(GetRandomValue(0, screenHeight - player.rec.height * 2));
		tube[1].rec.x = 400.0f;
		tube[1].rec.y = tube[0].rec.height + static_cast<float>(pipesDistance);
		tube[1].rec.width = 80.0f;
		tube[1].rec.height = static_cast<float>(screenHeight - tube[0].rec.height + pipesDistance);

		tube[2].rec.x = 700.0f;
		tube[2].rec.y = 0.0f;
		tube[2].rec.width = 80.0f;
		tube[2].rec.height = static_cast<float>(GetRandomValue(0, screenHeight - player.rec.height * 2));
		tube[3].rec.x = 700.0f;
		tube[3].rec.y = tube[2].rec.height + static_cast<float>(pipesDistance);
		tube[3].rec.width = 80.0f;
		tube[3].rec.height = static_cast<float>(screenHeight - tube[2].rec.height + pipesDistance);

		tube[4].rec.x = 1000.0f;
		tube[4].rec.y = 0.0f;
		tube[4].rec.width = 80.0f;
		tube[4].rec.height = static_cast<float>(GetRandomValue(0, screenHeight - player.rec.height * 2));
	...
	void PipesOutOfScreen()
	{
		for (int i = 0; i < sizePipes; i++)
		{
			...
			{
				tube[i].rec.x = static_cast<float>(screenWidth);
				tube[i].rec.y = 0.0f;
				tube[i].rec.height = static_cast<float>(GetRandomValue(0, screenHeight - player.rec.height * 2));
				i++;
				tube[i].rec.x = static_cast<float>(screenWidth);
				tube[i].rec.y = tube[i - 1].rec.height + pipesDistance;
				tube[i].rec.height = static_cast<float>(screenHeight - tube[i - 1].rec.height + pipesDistance);
				player.points++;
				PlaySound(pointSFX);
			}
		}
