	...
	void PipesOutOfScreen() 
	{
		for (int i = 0; i < sizePipes; i++)
		{
			if (tube[0].rec.x <= minScreenWidth - playerWidth && tube[1].rec.x <= minScreenWidth - playerWidth)
			{
				tube[0].rec.x = 800.0f;
				tube[0].rec.y = 0.0f;
				tube[0].rec.height = GetRandomValue(200, 250);
				tube[1].rec.x = 800.0f;
				tube[1].rec.y = 400.0f;
				tube[1].rec.height = GetRandomValue(50, 100);
			}
			if (tube[2].rec.x <= minScreenWidth - playerWidth && tube[3].rec.x <= minScreenWidth - playerWidth)
			{
				tube[2].rec.x = 1100.0f;
				tube[2].rec.y = 0.0f;
				tube[2].rec.height = GetRandomValue(200, 250);
				tube[3].rec.x = 1100.0f;
				tube[3].rec.y = 400.0f;
				tube[3].rec.height = GetRandomValue(50, 100);
			}
		}
	}
